...
export const worldScout: ScoutAgent = {
  ...
  async executeClusters(clusters: EventCluster[], config?: ScoutConfigOptions): Promise<ScoutResult> {
    ...
    for (const cluster of clusters) {
      ...
      const matchedSignals = detectSignals(clusterText, WORLD_SIGNALS);

      const anchorStory = cluster.stories[0];
      if (cluster.primaryCategory === 'world' || matchedSignals.length > 0) {
        const { score, breakdown, reason } = calculateSelectionScore(anchorStory, 'world', matchedSignals);
