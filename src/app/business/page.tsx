...
export default function BusinessPage() {
  ...
  const filtered = stories.filter(story => {
    const isCategory = story.category === 'business';
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-900 pb-4 gap-2">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-amber-400 font-mono flex items-center gap-2">
            <TrendingUp className="w-5 h-5" />
            BUSINESS & COMMERCE INTEL
          </h1>
          <p className="text-xs text-zinc-500 font-mono">SCOUT: BUSINESS SCOUT // LOGS SYNCED</p>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center border border-dashed border-zinc-800 rounded-lg py-20 px-4 bg-zinc-900/5 text-center">
          <h3 className="text-sm font-semibold text-zinc-400 font-mono">No Matching Business Signals</h3>
        </div>
      ) : (
