...
export default function AiTechPage() {
  ...
  const filteredClusters = eventClusters.filter(cluster => {
    const isCategory = cluster.primaryCategory === 'ai-tech';
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10 font-sans">
      <div className="flex items-center justify-between border-b border-zinc-900 pb-4 font-mono">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-zinc-100 uppercase">AI & Technology Event Clusters</h1>
            <p className="text-xs text-zinc-500">ARTIFICIAL INTELLIGENCE // SEMICONDUCTORS // CYBERSECURITY</p>
          </div>
        </div>
      ...
      {filteredClusters.length === 0 ? (
        <div className="border border-dashed border-zinc-800 rounded-lg p-12 text-center font-mono text-zinc-500 text-xs">
          No AI & Tech Event Clusters detected.
        </div>
      ) : (
